...
<main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
  <!-- Navbar -->
  <nav class=" mt-3 navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl " id="navbarBlur" data-scroll="true">
    <div class="container-fluid py-1 px-3">
      <nav aria-label="breadcrumb">
        <h4 class="font-weight-bolder mb-0">Menus</h4>
      </nav>
      <div class="" id="navbar">
    ...
    <div class="row">
      <div class="d-flex justify-content-end">
        <button class="btn btn-primary"
          ...
          title="New Menu" route={{ route('admin.menus.store') }}
                ...
                <tbody>
                  @foreach ($menus as $menu)
                    <tr>
                      ...
                      <td>
                        {{ \Carbon\Carbon::parse($menu->serving_time_start)->format('h:i A') . " - " . \Carbon\Carbon::parse($menu->serving_time_end)->format('h:i A')}}
                      </td>
                      <td class="align-middle text-center text-sm">
                        <span class="badge badge-sm bg-gradient-success">{{$menu->status}}</span>
                      </td>
                      <td class="d-flex gap-2">
                        <a class="btn btn-outline-secondary px-2 py-1 font-weight-bold text-xxs" 
                          data-bs-toggle="modal" data-bs-target="#menu" data={!! json_encode($menu) !!}
                          title="Edit Menu" route={{ route('admin.menus.update', ['id' => $menu->id]) }}>

                          Edit
                        </a>
                        <a class="btn btn-outline-danger px-2 py-1 font-weight-bold text-xxs" 
                          href={{ route('admin.menus.destroy', ['menu' => $menu->id]) }}>
                          Delete
                        </a>
                      </td>
                    </tr>  
  ...
  {{-- Modal --}}
  <div class="modal fade" id="menu" tabindex="-1" role="dialog" aria-labelledby="modal-form" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="card-body">
                    ...
                    <div class="text-center">
                        <button type="submit" class="btn btn-round bg-gradient-info btn-lg w-100 mt-4 mb-0">Save Menu</button>
                    </div>
                </div>
                </div>
            </div>
  ...
  <script>
    ...
    document.getElementById('menu').addEventListener('show.bs.modal', function(event){
        ...
        if (data == null){
            modal.querySelector('input[name=serving_time_start]').value = "{{ \Carbon\Carbon::parse('8:00 AM')->format('H:i') }}"
            modal.querySelector('input[name=serving_time_end]').value = "{{ \Carbon\Carbon::parse('9:00 PM')->format('H:i') }}"
        }
        else{
